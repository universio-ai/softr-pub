  ...
  setTimeout(() => cwtSafetyPulse(), 3000);
        ...
        async function sendActiveDelta(deltaMs, final = false) {
            ...
            try {
                ...
                if (j && j.updated && typeof j.updated.node_percent === "number") {
                    const raw = Number(j.updated.node_percent);
                    if (!Number.isNaN(raw)) {
...
`);
        ...
        async function renderFirstTimeWelcome(options = {}) {
            ...
            const welcomeSnippet = "go back to the Dashboard and hit Explore";
            if (
                conversation.some(
                    (msg) =>
                        msg?.sender === "tutor" &&
                        typeof msg?.text === "string" &&
                        (msg.text === welcomeText || msg.text.includes(welcomeSnippet))
                )
            )
                return false;
